// Phase 0 编辑器框架重构专项验证 · 组卷页（ExamBuilder 迁移）
// 本地 dev :5173 + staging 后端。覆盖 P0-3 左栏基本信息卡 / P0-4 左栏底框架小微 / 业务渲染不崩
// 用法：BASE=http://localhost:5173 node verify_editor_p0_exam.cjs
...
async function main() {
  ...
  record('组卷页渲染健康', (!h.appErr && !h.redir && h.visible) ? 'PASS' : 'FAIL', `visible=${h.visible} appErr=${h.appErr} redir=${h.redir} len=${h.len}`)

  const lp = await page.evaluate(() => {
    const t = document.body.innerText
    return { basic: /基本信息/.test(t), subj: /学科/.test(t), cls: /班级/.test(t), grade: /年级/.test(t), title: /试卷标题/.test(t) }
  })
  record('P0-3 左栏基本信息卡 + 试卷标题', (lp.basic && lp.subj && lp.cls && lp.grade && lp.title) ? 'PASS' : 'FAIL', JSON.stringify(lp))

  const biz = await page.evaluate(() => {
    const t = document.body.innerText
    return { draft: /保存为草稿/.test(t), pub: /发布/.test(t), know: /知识点范围/.test(t) }
  })
  record('业务渲染(保存草稿/发布/知识点范围)', (biz.draft && biz.pub && biz.know) ? 'PASS' : 'FAIL', JSON.stringify(biz))
  ...
  if (fail === 0) console.log('P0_EXAM_PASS')
  await browser.close()
}
