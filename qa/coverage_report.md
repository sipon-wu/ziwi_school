...
脚本：`qa/e2e_realistic_courseware.cjs`（Playwright 真机 + 真实 token 13800000002，每次随机抽取 2 个不同学科真实课题，走完「AI 生成课件 → 课前问诊作答 → AI 润色提纲(render-ppt) → 播放/阅读 → 导出 PPT → 保存到素材库」全链路；**数据保留、不删除**）。运行痕迹追加至 `qa/realistic_runs.log`，导出 PPT 落 `qa/downloads/`。

### 真实浏览器验收结果（预发布，本轮 PASS=true，0 页面错误）
...
丰富性判据（断言阈值）：生成提纲页数 ≥ 5、要点总数 ≥ 10、润色后仍 ≥ 5 页、播放幻灯片 ≥ 6 页、导出 PPT > 0 字节、保存到素材库成功——全部 PASS。四份课件均真实保留在预发布素材库（不同时间戳，可重复验收）。

### 验证要点说明
- 拟真真实性：随机抽课题、填真实课题名+附加要求、开启边缘知识（科学探究精神）、课前问诊逐题作答（8 题），等价于真人操作。
- 丰富性由 DOM 实测：提纲页数 = 预览区 `P{n}` 标签数；要点数 = 每页 `每条要点一行` 文本域行数之和；播放页数 = `PPT 在线预览` 的 `{idx+1} / {total}`。
- 导出 PPT 由前端 pptxgenjs 生成并触发下载，实测文件 153–155KB，非空。
- 入库以真实 API `GET /api/materials` 回查 `name` 含课题标题验证（曾误用 SPA 路由 `/materials` 致验收误判，已修正为 `/api/materials`）。

### 状态说明
- 仅落预发布，生产未部署。四份拟真课件为真实产出、保留在预发布素材库，可供后续人工验收。
- 如需更多学科/课题覆盖，重复运行 `node qa/e2e_realistic_courseware.cjs` 即生成新一批拟真课件（数据持续累积保留）。
