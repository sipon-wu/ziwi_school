    ...
    <aside className="w-[200px] bg-sidebar flex flex-col shrink-0">
      ...
      <nav className="flex-1 py-2 overflow-auto">
        {navItems.map(({ to, icon: Icon, label }) => {
          ...
          return (
            <Link key={to} to={to} className={`flex items-center gap-3 px-4 h-11 text-[13px] transition-colors duration-150 ${active ? 'bg-brand/30 text-white border-l-[3px] border-brand' : 'text-white/70 hover:bg-white/10 border-l-[3px] border-transparent'}`}>
