...
export function printExamPaper(questions: ExamQuestion[], meta: { subject:string; grade:string; title:string; difficulty:string; teacherName:string }) {
  ...
  const questionsHtml = questions.map(q => {
    qi++
    let html = `<div class="question"><strong>${qi}. [${typeName[q.type] || '题目'}]</strong> ${q.content} <em>(${Math.round(100/questions.length)}分)</em></div>`
    if (q.type === 'choice' && q.options?.length) {
      html += `<div class="options">${q.options.map((o,i) => `<div class="option">${String.fromCharCode(65+i)}. ${o}</div>`).join('')}</div>`
    }
...
<style>
  ...
  .question { font-weight:500; margin:16px 0 6px; }
...
export function printLessonPlan(content: string, meta: { subject:string; grade:string; title:string; teacherName:string; textbookUnit?:string }) {
  const html = content
    ...
    .replace(/^\|.+\|$/gm, m => `<div class="table-row">${m.split('|').filter(Boolean).map(c=>`<span>${c.trim()}</span>`).join('')}</div>`)
...
<style>
  ...
  .table-row { display:flex; gap:8px; padding:2px 0; }
