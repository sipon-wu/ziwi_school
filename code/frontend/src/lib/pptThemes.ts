...
/**
 * 装饰风格：从丰富版式中提炼出的「视觉识别维度」，由 groupId 自动归类。
 * 这是模板的「辨识度」来源——同一色系下不同 decor 也呈现不同版式气质。
 *  - china  国风：竖排标题线、印章角标、留白
 *  - tech   科技：网格底纹、几何角标、发光描边
 *  - fresh  清新：圆角卡片、柔色块、轻点缀
 *  - academic 学术：细线分隔、规整栏框
 *  - warm   典雅：细金线、柔角标
 *  - gradient 现代：大色块、流光角标
 *  - special 专项：黑板/涂鸦/卡通等专属造型
 *  - minimal 简约：极简线条、留白
 */
export type DecorStyle = 'china' | 'tech' | 'fresh' | 'academic' | 'warm' | 'gradient' | 'special' | 'minimal'
...
function withDecor(t: Omit<CwTheme, 'decor' | 'font'> & { font?: string; decor?: DecorStyle }): CwTheme {
  ...
  return { ...t, decor: t.decor ?? d.decor, font: t.font ?? d.font }
}

const RAW: Omit<CwTheme, 'decor' | 'font'>[] = [
  // ── A. 中国风（语文 / 历史 / 传统文化） ──
...
const THEMES: CwTheme[] = RAW.map(withDecor)
