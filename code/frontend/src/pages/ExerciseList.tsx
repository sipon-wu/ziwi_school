...
import { FileSpreadsheet, Plus, Search, Calendar, CheckCircle, Clock } from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 bg-white p-3 rounded-xl border border-gray-200">
        <div className="flex-1 relative">
          ...
          <input type="text" placeholder="搜索作业..." className="w-full pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1A3A6B]/20" />
        </div>
        <select className="px-3 py-2 text-sm border border-gray-200 rounded-lg">
          <option>全部学科</option><option>语文</option><option>数学</option><option>英语</option>
        </select>
        <select className="px-3 py-2 text-sm border border-gray-200 rounded-lg">
          <option>全部难度</option><option>L1-基础</option><option>L2-中等</option><option>L3-提高</option>
        </select>
      </div>
                ...
                <tr key={item.id} className="hover:bg-gray-50/50 cursor-pointer">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${
                      item.subject === '语文' ? 'bg-blue-100 text-blue-700' : item.subject === '数学' ? 'bg-orange-100 text-orange-700' : 'bg-green-100 text-green-700'
                    }`}>{item.subject}</span>
                  </td>
                  <td className="px-4 py-3 text-sm text-gray-600">{item.class}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${
                      item.difficulty === 'L1' ? 'bg-green-50 text-green-600' : item.difficulty === 'L2' ? 'bg-yellow-50 text-yellow-600' : 'bg-red-50 text-red-600'
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-2">
                      <div className="w-24 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div className="h-full bg-[#1A3A6B] rounded-full" style={{ width: `${(item.submitted/item.total)*100}%` }} />
                      </div>
                      <span className="text-xs text-gray-500">{item.submitted}/{item.total}</span>
                    </div>
                  </td>
                </tr>
              ))}
