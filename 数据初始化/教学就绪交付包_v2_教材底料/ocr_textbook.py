# -*- coding: utf-8 -*-
"""
教材正文页 OCR 流水线 (云端批作业形态 / cloud batch, in-memory)
================================================================
架构 (对应 "先存图片链接, 云端小批量长周期OCR"):
  - 图片链接已存于 output/raw_tch_material.json (每页 preview URL), 本脚本不下载/不存原图。
  - OCR 全程【内存】进行: 拉取 URL -> bytes -> cv2 解码进内存 -> RapidOCR(np数组) -> 丢弃 bytes。
...
两种用法:
  1) 单本/抽检 (本地验证):
     ocr_env/Scripts/python.exe ocr_textbook.py --bookid <id> [--keep 3]
  2) 云端小批量长周期批作业:
     ocr_env/Scripts/python.exe ocr_textbook.py --batch-file 教材OCR_小学语数英优先档.csv \
            --batch 20 --done-file ocr_done.jsonl
...
OUT_JSONL = ROOT / "output/底料_教材OCR.jsonl"
OUT_SUMMARY = ROOT / "output/底料_教材OCR_summary.json"
...
def main():
    ...
    ap.add_argument("--done-file", default="output/ocr_done.jsonl", help="已完成 bookId 检查点")
    ...
    if args.batch_file:
        ...
        for i, bid in enumerate(todo, 1):
            ...
            print(f"  [{i}/{len(todo)}] {title}: OCR {len(chunks)} 页, 失败 {len(failed)}, 累计{time.time()-t_all:.0f}s", flush=True)
        print(f"\n=== 本轮结束: 处理 {len(todo)} 本, 总耗时 {time.time()-t_all:.0f}s ===")
        print(f"检查点: {done_path}")
        return

    print("用法:\n  --bookid <id> [--keep 3]\n  --batch-file <csv> --batch 20 --done-file ocr_done.jsonl")
...
def _emit(chunks, title, bid, tg, prev, failed, pd):
    ...
    summ = {
        ...
        "学制": f"{tg.get('zxxxd','')}/{tg.get('zxxxk','')}/{tg.get('zxxnj','')}/{tg.get('zxxcc','')}/{tg.get('zxxbb','')}",
        "预览页总数": len(prev), "OCR成功页": len(chunks),
        "OCR失败/空页": len(failed),
        "总OCR字符数": sum(p["字符数"] for p in pd),
